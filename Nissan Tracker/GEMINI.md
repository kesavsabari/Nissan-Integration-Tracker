...
## 🛠 Tech Stack
- **Backend:** Python `http.server` (Zero-dependency mode).
...
- **Server Modernization:** Implemented built-in `gzip` compression in `server.py`.
- **UI Modernization:**
...
- **New Features:**
  ...
  - **Automatic Backups:** Throttled backup system in `server.py` that keeps the last 10 versions of all data files in a `backups/` folder.
